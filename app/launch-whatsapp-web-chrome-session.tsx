...
export function LaunchWhatsappChromeSession({ loginStatus, setLoginStatus }: Props) {
  ...
  const launchChrome = async () => {
    try {
      ...
      // 
      const responsew = await fetch('/api/chrome', { method: 'DELETE' });
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        if (data.status === 'connected') {
          logger.log('debugger connected, proceeding to Whatsapp Web');
          setStatus('connected');
            ...
            {(status === 'idle' || status === 'connecting') && (
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {status === 'connecting' ? 'launching chrome...' : 'Launch Whatsapp Web'}
                </Button>
              </div>
            )}
            {status === 'connected' && (
